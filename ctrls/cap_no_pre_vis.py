...
class CapCtrl:
    ...
    def set_encode_switch(self, encoder_switch: int) -> bool:
        ...
        if self.config["setting"]["storage_params"]["video_name"] == "/dev/null":
            ...
            video_max_files = self.compute_free_space(mount_point, "video", dur) or 0
        ...
        if self.config["setting"]["storage_params"]["image_name"] == "/dev/null":
            ...
            mount_point = str(Path(parent_path).parents[1])
            image_max_files = self.compute_free_space(mount_point, "image") or 0
            image_location = f"{output_path_image}/{self.config['setting']['storage_params']['image_name']}"

        udp_ip = self.config["setting"]["encoder_params"]["udp_ip"]
        udp_port = self.config["setting"]["encoder_params"]["udp_port"]
        v_ip = self.config["setting"]["storage_params"]["video_ip"]
        v_port = self.config["setting"]["storage_params"]["video_port"]
        i_ip = self.config["setting"]["storage_params"]["image_ip"]
        i_port = self.config["setting"]["storage_params"]["image_port"]
        ...
        params = {
            ...
            "udp_clients": f"{udp_ip}:{udp_port},{v_ip}:{v_port},{i_ip}:{i_port}",
        }

        # 1) 开启实时编码推流
        if not self.imager_manager.assign_switch_worker("live_encoder", **params):
            self.logger.error("启动编码推流失败")
            return False
    ...
    def set_encode_setting(self, kbps: int, fps: int) -> bool:
        # 0x20
        ...
        self.config["setting"]["encoder_params"]["bps"] = int(kbps * 1000 * 0.6)
        ...
        params = {
            ...
            "fontdesc": self.config["setting"]["encoder_params"]["fontdesc"],
        }
    ...
    def set_video_storge(self, video_storage_switch: int) -> bool:
        # 0x21

        """
        视频存储开关：
        0 -> 关闭存储（video_name/image_name 置为 /dev/null，并停止分段存储 worker）
        1 -> 开启存储（设置文件名模板，创建输出目录，计算可用配额，下发到 live_encoder）
        """

        self.logger.debug(f"视频存储开关: {video_storage_switch}")

        sp = self.config["setting"]["storage_params"]
        ep = self.config["setting"]["encoder_params"]

        # —— 关闭存储 —— #
        if video_storage_switch == 0:
            prev_closed = (sp.get("video_name") == "/dev/null" and sp.get("image_name") == "/dev/null")

            # 停止分段存储 worker
            if not self.imager_manager.cancel_worker("storage_splitvideo"):
                self.logger.error("停止分段存储失败: storage_splitvideo")
                return False

            # 若之前不是关闭，重发一次 live_encoder，把配额清零 & 路径指向 /dev/null
            if not prev_closed:
                # 更新配置为关闭
                sp["video_name"] = "/dev/null"
                sp["image_name"] = "/dev/null"
                ...
                params = {
                    ...
                    "bps": ep["bps"],
                    "udp_clients": (
                        f"{ep['udp_ip']}:{ep['udp_port']},"
                        f"{sp['video_ip']}:{sp['video_port']},"
                        f"{sp['image_ip']}:{sp['image_port']}"
                    ),
                    "fontdesc": ep.get("fontdesc", "Sans, 0"),
                }
                if not self.imager_manager.assign_worker("live_encoder", **params):
                    self.logger.error("关闭存储时更新 live_encoder 失败: %s", params)
                    return False

            self.logger.info("已关闭视频/图像存储（%s）", "重复关闭不重发" if prev_closed else "已重发live_encoder清零")
            return True

        # —— 开启存储 ——
        if sp.get("video_name") == "video_%Y%m%d_%H%M%S.h264" and sp.get("image_name") == "image_%Y%m%d_%H%M%S.jpg":
            self.logger.warning("视频存储已开启, 无需重复操作!")
            return True
        
        # 设置文件模板
        sp["video_name"] = "video_%Y%m%d_%H%M%S.h264"
        sp["image_name"] = "image_%Y%m%d_%H%M%S.jpg"

        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")

        # —— 计算 video location & quota —— #
        # 文件名模板与路径
        video_filename_tpl = sp["video_name"]          # e.g. "video_%Y%m%d_%H%M%S.h264"
        video_path_base   = sp["video_path"]           # 根目录
        video_output_dir  = os.path.join(video_path_base, "auto_save", "video", timestamp)
        ...
        # 找挂载点（尽量健壮）
        try:
            mount_point_video = str(Path(video_path_base).parents[1])
        except Exception:
            # 兜底：退到根挂载
            mount_point_video = str(Path(video_path_base).anchor or "/")
        ...
        # 可存储的段文件数
        try:
            video_max_files = int(self.compute_free_space(mount_point_video, "video", dur))
        except Exception:
            self.logger.exception("计算视频可用空间失败: mount=%s, dur=%s", mount_point_video, dur)
            video_max_files = 0
        if video_max_files <= 0:
            video_max_files = 0

        video_location = f"{video_output_dir}/{video_filename_tpl}"

        # —— 计算 image location & quota —— #
        image_filename_tpl = sp["image_name"]          # e.g. "image_%Y%m%d_%H%M%S.jpg"
        image_path_base   = sp["image_path"]
        image_output_dir  = os.path.join(image_path_base, "auto_save", "image", timestamp)

        try:
            mount_point_image = str(Path(image_path_base).parents[1])
        except Exception:
            mount_point_image = str(Path(image_path_base).anchor or "/")

        os.makedirs(image_output_dir, exist_ok=True)

        try:
            image_max_files = int(self.compute_free_space(mount_point_image, "image"))
        except Exception:
            self.logger.exception("计算图像可用空间失败: mount=%s", mount_point_image)
            image_max_files = 0
        if image_max_files <= 0:
            image_max_files = 0

        image_location = f"{image_output_dir}/{image_filename_tpl}"

        # —— 汇总参数，下发到 live_encoder —— #
        # UDP 客户端三元：编码预览、视频存储、图像存储
        udp_clients = (
            f"{ep['udp_ip']}:{ep['udp_port']},"
            f"{sp['video_ip']}:{sp['video_port']},"
            f"{sp['image_ip']}:{sp['image_port']}"
        ...
        params = {
            "video_max_files": video_max_files,
            "video_location":  video_location if video_max_files > 0 else "/dev/null",
            "image_max_files": image_max_files,
            "image_location":  image_location if image_max_files > 0 else "/dev/null",
            "bps": ep["bps"],
        ...
        if not self.imager_manager.assign_worker("live_encoder", **params):
            self.logger.error("更新视频备份参数失败: %s", params)
            return False

        self.logger.info(
            "已开启视频/图像存储: video(%s × %s), image(%s × %s)",
            params["video_location"], params["video_max_files"],
            params["image_location"], params["image_max_files"],
        )
        return True
    ...
    def set_video_storge_setting(self, kbps: int, fps: int, dur: int) -> bool:
        ...
        params = {
            ...
            "output_path": f"{output_path}/video_%Y%m%d_%H%M%S.h264",
        }
    ...
    def set_detectmode(
        ...
        # 1. 关闭原始数据推流
        # 2. 开启有appsink tee和cario的流水线
        # 3. 推理

        if not self.imager_manager.cancel_worker("live_encoder"):
            self.logger.error("关闭旧流水线失败")
            return False
        ...
        if not self.imager_manager.assign_worker("live_det_encoder", **params): # 创建self.queues["det"]
        ...
        try:
            ...
            if not self.imager_manager.assign_detworker(*start_args):   # 传入self.queues["det"]
                self.logger.error("目标检测启动失败")
                return False

        except Exception:
            self.logger.exception("检测流程异常")
            return False
    ...
    def set_detectstop(self) -> bool:
        ...
        if "live_det_encoder" not in self.imager_manager.active_workers:
            self.logger.debug("该相机还没有开启目标检测，无需停止！")
            return True

        if not self.imager_manager.cancel_worker("live_det_encoder"):
            return False

        if not self.imager_manager.cancel_detworker():
            return False

        if not self.set_encode_switch(1):
    ...
    def set_cross_line(self, iscrossline_enable: int) -> bool:
        ...
        fontdesc = (
            "Sans UltraCondensed 20"
            if iscrossline_enable == 1
            else "Sans UltraCondensed 0"
        )
        self.config["setting"]["encoder_params"]["fontdesc"] = fontdesc
        ...
        if self.config["setting"]["storage_params"]["image_name"] == "/dev/null":
            ...
            image_location = f"{output_path_image}/image_%Y%m%d_%H%M%S.jpg"
